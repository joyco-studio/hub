...
`Canvas2D` is a plain `<canvas>` element that observes its own CSS size and keeps the buffer resolution and DPR in sync. Size it however you want — Tailwind classes, inline styles, flex/grid — no wrapper needed.
...
**Returns:** `[canvasRef, { ctx, width, height, dpr }]`

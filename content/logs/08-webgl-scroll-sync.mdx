...
C["GPU shifts pixel tiles<br/>instantly"]:good
D["Main thread notified<br/>asynchronously"]:bad
A --- B
B --- C
B --- D`}</Diagram>
...
CO["Copy layer tree +<br/>property trees to impl thread<br/>(main thread blocked)"]:pending
end
subgraph impl ["Compositor Thread (impl)"]
LY["Layerize"]:good
R["Raster<br/>(worker threads → GPU tiles)"]:good
AC["Activate"]:good
DR["Draw compositor frame"]:good
LY --- R
R --- AC
AC --- DR
end
...
LTHI["LayerTreeHostImpl<br/>owns impl-thread tree"]:good
end
...
PAD_TOP["25% padding (top)<br/>Extra rendered area"]:good
VP["100% viewport area<br/>What the user actually sees"]:primary
PAD_BOT["25% padding (bottom)<br/>Extra rendered area"]:good
PAD_TOP --- VP
VP --- PAD_BOT
end
SCROLL["During fast scroll,<br/>stale transform shifts canvas<br/>a few pixels"]
NOTE["Padding absorbs the shift<br/>→ no visible clipping"]:good
SCROLL --- VP
NOTE --- VP`}</Diagram>
...
PROBLEM["Compositor scrolls instantly (GPU)<br/>JS reads scrollY 1+ frames late"]:pending
Q{"Where does<br/>the canvas live?"}:primary
FIXED["Window side (fixed)"]:primary
ABS["Page side (absolute)"]:good
FIX_HOW["JS-driven scroll (Lenis):<br/>JS animates scrollTop<br/>Canvas reads it back"]
FIX_PRO["Zero drift between<br/>DOM and canvas"]:good
FIX_CON["Main-thread scrolling<br/>Can drop frames on mobile"]:bad
ABS_HOW["Keep native scroll:<br/>Canvas moves with page<br/>Transform corrects position"]
ABS_PRO["Native scroll preserved<br/>Content stays aligned"]:good
ABS_CON["Edge clipping (fix with padding)<br/>Viewport-fixed WebGL elements drift"]:bad
ROOT --- PROBLEM
PROBLEM --- Q
Q --- FIXED
Q --- ABS

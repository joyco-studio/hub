...
└─────┬───────┘
      ...
      ▼
┌─────────────┐
│  Render HTML │  Everything waits 350ms+
└─────────────┘
```
...
Request arrives
      ...
      ▼                                  │
┌──────────────────────┐                 │
│  Render HTML shell   │ immediate       │
│  (with fallbacks)    │                 │
└──────────┬───────────┘                 │
           │                             │
           ▼                             ▼
┌──────────────────────────────────────────┐
│  Stream resolved data as promises settle │
└──────────────────────────────────────────┘
```
...
4. Send response         ──→     5. Display everything at once
                                  6. Hydrate

Time: ████████████████████░░░░  (slow TTFB, fast render)
```
...
1. Start getCart()
2. Start getFlags()
3. Render static shell   ──→     4. Display shell + fallbacks instantly
   (with fallback UI)            5. Hydrate static parts

   ...promises resolve...

6. Stream cart data      ──→     7. CartCount + CartTotal pop in
8. Stream flags data     ──→     9. CheckoutButton resolves

Time: ██░░░░░░░░░░░░░░░░░░░░░  (fast TTFB, progressive render)
```
...
```txt
 0ms    50ms   100ms  150ms  200ms  250ms  300ms
  │      │      │      │      │      │      │
  ├──────────────────────────────────────────────
  │ Static shell served (nav, main layout, footer)
  ├──────────────────────────────────────────────
  │
  ├─── flags resolve ──┤
  │                     └→ CheckoutButton, BetaBanner render
  │
  ├──────── cart resolves ─────┤
  │                            └→ CartCount, CartTotal render
  │
  ▼
...
│                        BUILD TIME                           │
│                                                             │
│  Next.js prerenders the route:                              │
│  ┌───────────────────────────────────────────────────────┐  │
│  │ Static Shell                                          │  │
│  │  • <html>, <body>, <nav>, <main>, <footer>            │  │
│  │  • Suspense fallbacks (skeletons, "Loading...")        │  │
│  │  • All non-dynamic content                            │  │
│  └───────────────────────────────────────────────────────┘  │
│  Stored on CDN / edge cache                                 │
└─────────────────────────────────────────────────────────────┘
                          │
                          ▼
┌─────────────────────────────────────────────────────────────┐
│                       REQUEST TIME                          │
│                                                             │
│  1. CDN serves static shell            ──→ Browser paints  │
│                                             immediately     │
│  2. Server executes layout:                                 │
│     ┌─────────────────────────────┐                         │
│     │ const cartPromise = getCart()│──→ fetch in flight      │
│     │ const flagsPromise = getFlags()│→ fetch in flight     │
│     └─────────────────────────────┘                         │
│                                                             │
│  3. Promises forwarded to client providers via RSC payload  │
│                                                             │
│  4. As each promise resolves, React streams the             │
│     component tree update to replace the fallback           │
│     ┌──────────────┐    ┌──────────────┐                    │
│     │ flags ready   │    │ cart ready    │                   │

...
B["await getCart()<br/>200ms"]:bad
C["await getFlags()<br/>150ms"]:bad
D["Render HTML"]
E["Send response<br/>350ms+ later"]:bad
A --- B
B --- C
C --- D
D --- E`}</Diagram>
...
A(["Request arrives"]):primary
B["getCart() started"]:good
C["getFlags() started"]:good
...
skeleton1["CartTotal skeleton"]:pending
skeleton2["CartCount skeleton"]:pending
main["<main> Featured Products"]:primary
fallback["Checkout button fallback"]:pending
shell --- nav
nav --- skeleton1
nav --- skeleton2
skeleton1 --- main
...
S["UI renders with<br/>Suspense fallbacks instantly"]:pending
B["getFlags resolves<br/>80ms"]:good
C["getCart resolves<br/>120ms"]:good
D["CheckoutButton renders<br/>Checkout with Express Pay"]:streamed
E["CartCount: 3<br/>CartTotal: $149.97"]:streamed
F(["Page fully interactive<br/>~120ms"]):good
A --- S
S --- B
S --- C
B === D
...
R2["Browser paints immediately"]:good

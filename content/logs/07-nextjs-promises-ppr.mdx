...
In Next.js 16, PPR is enabled by setting `cacheComponents: true` in your `next.config.ts`. No per-route opt-in is needed — the static shell is the default behavior. Combined with the `'use cache'` directive and `cacheLife`, you get the full static shell + streaming combo out of the box.

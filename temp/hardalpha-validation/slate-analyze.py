...
coverage = down(read('slate-white-silhouette-16'), 16)[:, :, 0] / 255
inside = coverage > .5
boundary = ((coverage > .001) & (coverage < .999)) | (dilate(inside, 1) & dilate(~inside, 1))
masks = {'edge': dilate(boundary, 1), 'band': dilate(boundary, 6)}
results = {'chrome': json.loads((root / 'slate-manifest.json').read_text())['chrome'], 'backgrounds': {}, 'inline_max_channel_delta': {}}
for background in ['white', 'gray', 'dark']:
    candidates = {v: read(f'slate-{background}-{v}-1') for v in ['a127', 'a1', 'rebuilt']}
    refs = {s: down(read(f'slate-{background}-a127-{s}'), s) for s in [8, 16, 32]}
    ...
    board.save(out / f'slate-{background}-comparison.png')
for v in ['a127', 'a1', 'rebuilt']:
    results['inline_max_channel_delta'][v] = float(np.max(np.abs(read(f'slate-{v}-inline') - read(f'slate-white-{v}-1'))))
(root / 'slate-results.json').write_text(json.dumps(results, indent=2) + '\n')
